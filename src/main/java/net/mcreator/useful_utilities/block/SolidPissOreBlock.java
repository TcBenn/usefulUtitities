...
public class SolidPissOreBlock extends UsefulUtilitiesElements.ModElement {
	@ObjectHolder("useful_utilities:solidpissore")
	public static final Block block = null;
	public SolidPissOreBlock(UsefulUtilitiesElements instance) {
		super(instance, 7);
	}
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			super(Block.Properties.create(Material.ROCK).sound(SoundType.SLIME).hardnessAndResistance(1f, 10f).lightValue(1).harvestLevel(2)
					.harvestTool(ToolType.PICKAXE));
			setRegistryName("solidpissore");
		}
			...
			if (world.getRedstonePowerFromNeighbors(new BlockPos(x, y, z)) > 0) {
				{
					...
					SolidPissOreRedstoneOnProcedure.executeProcedure($_dependencies);
				}
			} else {
			}
		}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("solidpissore", "solidpissore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 16), Placement.COUNT_RANGE, new CountRangeConfig(15, 0, 0, 256)));
		}
	}
}
